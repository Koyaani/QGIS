...
class QgsGeoPackageItemGuiProvider : public QObject, public QgsDataItemGuiProvider
{
  ...
  private:
    static bool handleDropGeopackage( QgsGeoPackageCollectionItem *item, const QMimeData *data, QgsDataItemGuiContext context );
    //! Compacts (VACUUM) a geopackage database
    static void vacuumGeoPackageDbAction( const QString &path, const QString &name, QgsDataItemGuiContext context );
